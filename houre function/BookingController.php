...
public function handleHourlyHire(Request $request)
{
    ...
    public function submitPassengerInfo(Request $request)
    {
        ...
        if ($request->isMethod('get')) {
            ...
            $distanceData = [];
            $isHourlyHire = session('service_type') === 'hourlyHire';
            foreach ($vehicles_all as $vehicle) {
                $distanceData[$vehicle->id] = $isHourlyHire
                    ? $this->calculateVehiclePrice($vehicle)
                    : $this->calculateDistanceBasedPrice($vehicle, (float) $distance);
            }
...
private function getDistanceBetweenAddresses(string $origin, string $destination): ?float
{
   ...
   // public function completeBook(Request $request)
    ...
    private function calculateVehiclePrice($vehicle, ?float $distanceMiles = null): array
    {
        if (session('service_type') === 'hourlyHire') {
            return $this->calculateDistanceWithStops(
                session('pickup_location'),
                null,
                json_decode(session('stops', '[]'), true) ?? [],
                $vehicle->base_fare,
                $vehicle->base_hourly_fare,
                $vehicle->per_km_rate,
                session('select_hours')
            );
        }
        ...
        if ($distanceMiles === null) {
            ...
            $dropoffLocation = session('dropoff_location');
            $stops = json_decode(session('stops', '[]'), true) ?? [];
            $distanceMiles = (float) session('route_distance_km', 0);
            if ($distanceMiles <= 0) {
                $distanceMiles = (float) $this->computeRouteDistance($pickupLocation, $dropoffLocation, $stops);
            }
        }

        return $this->calculateDistanceBasedPrice($vehicle, $distanceMiles);
    }
